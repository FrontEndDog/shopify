class VariantSelects extends HTMLElement {
  constructor() {
    ...
    this.isFullWidth =
      this.item.closest('.product-full-width').length == 1 || this.item.closest('.product-full-width-2').length == 1;
  ...
  updateMasterId() {
    ...
    if (
      this.item.find('[data-filter]').length &&
      this.currentVariant?.featured_media &&
      this.currentVariant.featured_media.alt != null
    ) {
      this.item
        .find('[data-filter]:checked')
        .attr('data-value-default-lang', this.currentVariant?.featured_media.alt.toLowerCase().replace(/ /g, '-'));
    }
  }
  ...
  updateMedia(time, status) {
    ...
    setTimeout(() => {
      if (
        !this.currentVariant ||
        !this.currentVariant?.featured_media ||
        document.querySelector('.productView-nav')?.matches('.media-filter')
      )
        return;
  ...
  updateProductInfo() {
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        this.querySelectorAll('fieldset')[1].innerHTML = html.querySelectorAll('fieldset')[1].innerHTML;
      });
  }
  ...
  getVariantData() {
    ...
    console.log('getVariantData', this.variantData);
    ...
    inputWrappers.forEach((option, index) => {
      ...
      const optionInputsValue =
        inputLength > 1
          ...
          : this.variantData.map((variantOption) => variantOption[`option${index + 1}`]);
      ...
      inputWrappers.forEach((element, inputIndex) => {
        ...
        inputList.forEach((item, index) => {
          ...
          if (image.length > 0) {
            ...
            if (imageSpan[index] != undefined && image[0].featured_image != null)
              imageSpan[index].style.backgroundImage = `url("${image[0].featured_image.src}")`;
            if (imageSpanImage[index] != undefined && image[0].featured_image != null)
...
class QuantityInput extends HTMLElement {
  ...
  getVariantData() {
    this.variantData =
      this.variantData || JSON.parse(document.querySelector('.product-option [type="application/json"]').textContent);
    return this.variantData;
  }
}
